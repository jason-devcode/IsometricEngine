...
public:
  ...
  void drawUVFace( 
    ...
    Texture* txFront, Texture* txLeft, Texture* txTop, 
    uint32_t visibilityMask 
  ) {
    double dx = B.m_x - A.m_x;
    double dy = B.m_y - A.m_y;
    ...
    int steps = fabs(dy);
    double uvVariation = 1.0 / steps;
    v1 = 0.0;
    
    double offsetZ = 0.0;
    double ZA = offsetZ + A.m_x + A.m_y;
    double ZB = offsetZ + B.m_x + B.m_y;
    double ZD = ZA;
    
    // reciprocos de Z a distribuir
    double recZA = 1 / ZA;
    double recZB = 1 / ZB;
    double recZD = recZA;

    // Variacion entre el reciproco de B y A
    double recZVariation = (recZB - recZA) / (dy != 0.0 ? dy : 1.0);

    for( int linePoint = 0; linePoint <= steps; ++linePoint ) {
      // Draw top face
      if( visibilityMask & TOP_FACE ) {
        // top triangle
        shape_drawer.drawHorizontalLineUV(y1, x1, x2, 1.0 - v1, 0.0, 1, v1, txTop);
        //shape_drawer.drawHorizontalLineUV(y3, x3, x4, 0.0, v1, 1.0 - v1, 1.0, txTop);
...
//        shape_drawer.drawHorizontalLine(C.m_y + linePoint + steps-1, x3+1, C.m_x-1, colorLeft);
  ...
  void drawUVFillCube(
    ...
    drawUVFace(topA, topB, topC, topD, txFront, txLeft, txTop, visibilityMask);
  }
